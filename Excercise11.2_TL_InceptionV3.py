...
#Create the model
inceptionv3_body = tf.keras.applications.InceptionV3(include_top=False, input_shape=
                                              IMAGE_SIZE, pooling='avg')

#output shape of 1 input image(1, 224, 224, 3) thru the vgg_body  
features = inceptionv3_body.predict(np.random.random([1] + list(IMAGE_SIZE)))
D = features.shape[1]
...
for X, y in train_gen:
    ...
    features = inceptionv3_body.predict(X)      
...
plot_loss_wrong_preds(model_history, "Loss Curves - Inception_FeatureTensor_normalized",
...
#save metrics
file_stream = open('results/Ex11.2_Inceptionv3_OpBat_15ep_Normal.txt', 'w')
print("Xception Model Summary\n----------\n", file=file_stream)  
inceptionv3_body.summary(print_fn=lambda x: print(x, file=file_stream)) 
