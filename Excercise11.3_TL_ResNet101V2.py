...
#Create the model
resnetv2_body = tf.keras.applications.ResNet101V2(include_top=False, input_shape=
                                              IMAGE_SIZE, pooling='avg')

#output shape of 1 input image(1, 224, 224, 3) thru the vgg_body  
features = resnetv2_body.predict(np.random.random([1] + list(IMAGE_SIZE)))
D = features.shape[1]
...
for X, y in train_gen:
    ...
    features = resnetv2_body.predict(X)      
...
plot_loss_wrong_preds(model_history, "Loss Curves - Resnet101v2_FeatureTensor_unnormalized",
...
#save metrics
file_stream = open('results/Ex11.3_Resnet101v2_OpBat_15ep.txt', 'w')
print("Xception Model Summary\n----------\n", file=file_stream)  
resnetv2_body.summary(print_fn=lambda x: print(x, file=file_stream)) 
