# -*- coding: utf-8 -*-
"""
Created on Fri Jul 24 08:50:52 2020
...
features = xception_net.predict(np.random.random([1] + list(IMAGE_SIZE)))
D = features.shape[1]
...
for X, y in train_gen:
    ...
    features = xception_net.predict(X)      
...
model_history = model.fit(X_train, y_train, epochs=50, validation_data=(X_test, 
                                                                        y_test))
...
#save metrics
file_stream = open('results/Ex11.1_Xception_OpBat_50ep.txt', 'w')
print("Xception Model Summary\n----------\n", file=file_stream)  
xception_net.summary(print_fn=lambda x: print(x, file=file_stream)) 
